...
- 防暴力破解（速率限制 + 递进式账户锁定）
- Markdown 渲染器升级为 markdown-it（插件化架构）
- 对话引号高亮（全角 `""`、半角 `""`、直角 `「」`）
- 自定义 XML 标签自动隐藏（`<Status_Bar>` 等不显示）
- ASCII art / box-drawing 表格格式保留
- 消息编辑功能（用户和 AI 消息均可编辑，直接影响上下文）
- 思考等级设置（自适应/低/中/高/极高），自动兼容 OpenAI / Claude / Gemini
- 聊天界面显示上下文预估 token 数
- 保留最近消息数支持无上限（0 = ∞）
- 上下文压缩
- Safari / iOS 兼容性优化
...
**【免责与授权声明】**
本项目基于 **[CC BY-NC 4.0（知识共享-署名-非商业性使用 4.0 国际许可协议）](./LICENSE)** 开源。原始项目由 [**STA1N156**](https://github.com/STA1N156) 创建。**明确禁止任何形式的商业化使用。** 任何使用者必须遵守该协议，尊重[**原作者**](https://github.com/STA1N156)的署名权。
